...
Instead of forcing a new memory architecture, it helps the agent:

- scan recent daily logs
- extract durable, high-value information
- update memory incrementally
- record each run in a lightweight dream log
- move gradually from manual runs to semi-automatic memory consolidation

## Why this exists

Many memory systems try to rebuild everything.

`Auto Dream Light` does the opposite:
...
- extracts decisions, durable facts, stable preferences, project milestones, and reusable lessons
